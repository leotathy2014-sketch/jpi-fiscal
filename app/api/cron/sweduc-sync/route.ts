...
import {createSweducAccessToken,getSweducActiveAcademicYear,listSweducStudentsWithToken,normalizeSweducHost,parseSweducCredentials,type SweducCredentials,type SweducStudentSummary} from "@/lib/sweduc";
...
export async function GET(request:NextRequest){
  ...
  let activeCredentials:SweducCredentials|undefined;let activeAccessToken="";let synced=0;let page=1;let lastPage=1;
  try{
    const [secretResult,configResult]=await Promise.all([
      supabase.rpc("get_sweduc_secret_service",{p_backend_secret:backendSecret}),
      supabase.from("sweduc_config").select("host").eq("id",true).single()
    ]);
    ...
    const activeYear=await getSweducActiveAcademicYear(activeCredentials.host);
    ...
    while(page<=lastPage&&page<=MAX_PAGES_PER_RUN){
      const listing=await listSweducStudentsWithToken(activeCredentials.host,token.accessToken,{page,ano_letivo_id:activeYear.id});
    ...
    return json({ok:true,academicYear:activeYear.year,synced,finished,nextPage:finished?null:page,message:finished?`Sincronização automática concluída com ${synced} matrícula(s).`:`Sincronização parcial concluída com ${synced} matrícula(s). Próxima execução continua atualizando.`});
  }catch(error){
