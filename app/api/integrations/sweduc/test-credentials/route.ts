...
import {createSweducAccessToken,getSweducStudentDetailsWithToken,listSweducStudentsWithToken,normalizeSweducHost,type SweducCredentials} from "@/lib/sweduc";
...
function safeError(error:unknown,credentials:SweducCredentials){
  ...
  for(const secret of [credentials.clientId,credentials.clientSecret,basic])if(secret)message=message.split(secret).join("[protegido]");
...
export async function POST(request:NextRequest){
  ...
  if([hostInput,clientId,clientSecret].some(value=>value.length>2000))return json({error:"Um dos dados ultrapassa o tamanho permitido."},400);
  ...
  try{
    ...
    return json({
      ok:true,
      message:`Credenciais e permissões confirmadas. A SWeduc permitiu consultar ${visible} matrícula(s). Nada foi salvo ou importado.`,
      checks:{oauth:"confirmed",studentList:"confirmed",studentDetails:details},
    });
  }catch(error){
    return json({error:safeError(error,credentials)},400);
  }
}
