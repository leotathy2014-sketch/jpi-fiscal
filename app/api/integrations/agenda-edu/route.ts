...
export async function GET(request:NextRequest){
  ...
  const production=data.agenda_edu_environment==="producao";
  const ready=Boolean(data.agenda_edu_documentacao_confirmada&&data.agenda_edu_credencial_configurada&&data.agenda_edu_ultimo_status==="conectado"&&data.agenda_edu_channel_id);
  return json({ok:true,ready,phase:data.agenda_edu_ultimo_status,environment:production?"production":"sandbox",schoolIdentifierConfigured:Boolean(data.agenda_edu_school_identifier),channelConfigured:Boolean(data.agenda_edu_channel_id),checklist:{baseLocal:true,documentation:Boolean(data.agenda_edu_documentacao_confirmada),credential:Boolean(data.agenda_edu_credencial_configurada),connectionTest:Boolean(data.agenda_edu_testada_em),parentMessaging:ready},message:ready?`Agenda Edu conectada e pronta na ${production?"plataforma oficial":"homologação"}.`:"Teste novamente a conexão para o sistema identificar e salvar automaticamente o canal de Mensagens."});
}
