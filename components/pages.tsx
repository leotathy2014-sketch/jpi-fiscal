...
function CommunicationsSettings({accessToken,onChanged,canEdit,section}:{accessToken:string|null;onChanged:(config:CommunicationConfig)=>void;canEdit:boolean;section:"email"|"whatsapp"|"manual-whatsapp"|"agenda"}) {
  ...
  const sectionCopy={
    ...
    agenda:{title:"Agenda Edu",description:"Fonte de dados escolares para carregar alunos e responsáveis usados na emissão da NFS-e.",Icon:CalendarDays},
  }[section];
  ...
  return <div className="integration-detail-panel">
    ...
    {loading?<div className="communications-loading">Carregando configuração…</div>:<div className="integration-single-content">
      ...
      {section==="agenda"&&<><div className="integration-agenda-stack"><form className="communication-channel-card data-form" onSubmit={saveAgenda}>
          <div className="communication-channel-head"><span className="integration-icon purple"><CalendarDays/></span><div><h3>Agenda Edu</h3><small>Fonte de dados para emissão de NFS-e</small></div><IntegrationStateBadge label={sectionState.label} tone={sectionState.tone}/></div>
          <div className="notice compact"><ShieldCheck/><span>A API da Agenda Edu é usada somente para consultar e carregar dados escolares no JPI Fiscal. Esta integração não envia NFS-e nem mensagens para os responsáveis.</span></div>
          ...
          <div className="smtp-server-summary"><span>Ambiente da API</span><strong>Sandbox</strong><small>sandbox-api.agendaedu.dev · leitura e validação dos dados antes do uso em produção.</small></div>
          ...
          <div className="agenda-checklist"><span className="done"><Check/>Base local e rota protegida</span><span className="done"><Check/>Documentação oficial da API</span><span className={config?.agenda_edu_credencial_configurada?"done":""}>{config?.agenda_edu_credencial_configurada?<Check/>:<Clock3/>}Credenciais da API</span><span className={config?.agenda_edu_testada_em?"done":""}>{config?.agenda_edu_testada_em?<Check/>:<Clock3/>}Teste de acesso aos dados</span><span className={config?.agenda_edu_ultimo_status==="conectado"?"done":""}>{config?.agenda_edu_ultimo_status==="conectado"?<Check/>:<Clock3/>}Fonte de dados conectada</span></div>
          <button className="primary full" disabled={disabled}>{busy==="save-agenda"?"Salvando…":"Salvar configuração da Agenda Edu"}</button>
          <button type="button" className="secondary full" onClick={testAgenda} disabled={disabled||!config?.agenda_edu_credencial_configurada}>{busy==="test-agenda"?"Testando acesso…":"Testar acesso à API"}</button>
        ...
        <section className="agenda-data-source-card">
          <div className="agenda-api-section-title"><GraduationCap/><div><strong>Dados usados pelo JPI Fiscal</strong><small>A Agenda Edu será a fonte para preencher os cadastros necessários à emissão das notas.</small></div></div>
          <div className="agenda-data-source-grid">
            <span><Check/>Aluno</span><span><Check/>Responsável</span><span><Check/>Turma</span><span><Check/>Segmento</span><span><Check/>CPF/CNPJ, quando disponível</span><span><Check/>Contatos e endereço, quando disponíveis</span>
          </div>
          <p>Na sincronização, o JPI Fiscal fará o vínculo dos dados recebidos da Agenda Edu com o cadastro fiscal local, sem alterar os dados na Agenda Edu.</p>
        </section></div>
        {isMaster&&agendaApiInfoOpen&&<div className="agenda-api-modal-backdrop" role="presentation" onMouseDown={event=>{if(event.currentTarget===event.target){setAgendaApiInfoOpen(false);hideAgendaMasterSecrets();}}}>
          <section className="agenda-api-modal" role="dialog" aria-modal="true" aria-labelledby="agenda-api-modal-title">
            <header className="agenda-api-modal-head">
              <div className="agenda-api-modal-title"><span><KeyRound/></span><div><small>ACESSO EXCLUSIVO DO MASTER</small><h2 id="agenda-api-modal-title">Informações da API e credenciais</h2><p>Dados salvos no servidor para consulta, manutenção e sincronização dos cadastros usados pelo JPI Fiscal.</p></div></div>
            ...
            <div className="agenda-api-modal-body">
              <section className="agenda-api-status-card">
                <div className="agenda-api-status-copy"><span className={config?.agenda_edu_ultimo_status==="conectado"?"connected":config?.agenda_edu_ultimo_status==="erro"?"error":"pending"}></span><div><small>STATUS DA INTEGRAÇÃO</small><strong>{config?.agenda_edu_ultimo_status==="conectado"?"Acesso à API confirmado":config?.agenda_edu_ultimo_status==="erro"?"Acesso à API com erro":"Acesso à API pendente"}</strong><p>{config?.agenda_edu_ultimo_status==="conectado"?"Credenciais validadas e servidor acessível para leitura dos dados escolares.":"Execute o teste de acesso para validar a conexão."}</p></div></div>
              ...
              <div className="agenda-api-modal-grid">
                <section className="agenda-api-info-section">
                  ...
                  <dl className="agenda-api-data-list">
                    ...
                    <div><dt>ID retornado no teste</dt><dd className="mono">{config?.agenda_edu_channel_id||"Não informado"}</dd></div>
