    ...
    {loading?<div className="communications-loading">Carregando configuração…</div>:<div className="integration-single-content">
      ...
      {section==="agenda"&&<><div className="integration-agenda-stack"><form className="communication-channel-card data-form" onSubmit={saveAgenda}>
          ...
          {isMaster&&canEdit&&<section className="communication-channel-card agenda-diagnostic-card">
            <div className="communication-channel-head"><span className="integration-icon purple"><MessageCircle/></span><div><h3>Lista de canais Agenda Edu</h3><small>Primeiro passo do módulo Mensagens: identificar o canal correto.</small></div><IntegrationStateBadge label={agendaChannelList?"Carregado":"Consulta"} tone={agendaChannelList?"connected":"pending"}/></div>
            <div className="notice compact"><ShieldCheck/><span>Consulta somente `GET /channels`. Use para confirmar qual canal contém as turmas/famílias antes de procurar chats e mensagens.</span></div>
          ...
          {isMaster&&canEdit&&<section className="communication-channel-card agenda-diagnostic-card">
            <div className="communication-channel-head"><span className="integration-icon green"><MessageCircle/></span><div><h3>Localizar chat familiar</h3><small>Teste do caminho canal + aluno antes de criar mensagem.</small></div><IntegrationStateBadge label={agendaFamilyChat?agendaFamilyChat.found?"Encontrado":"Não encontrado":"Consulta"} tone={agendaFamilyChat?.found?"connected":"pending"}/></div>
            <div className="notice compact"><ShieldCheck/><span>Consulta somente `GET /channels/{'{'}canal{'}'}/chats`. Não cria chat, não envia nota e não grava vínculo.</span></div>
            <div className="form-row"><label>ID do canal<input value={agendaChatChannelId||agendaEduChannelId} onChange={event=>setAgendaChatChannelId(event.target.value.replace(/[^A-Za-z0-9._-]/g,""))} placeholder="Ex.: 68347"/></label><label>ID do aluno Agenda Edu<input value={agendaChatStudentId} onChange={event=>setAgendaChatStudentId(event.target.value.replace(/[^A-Za-z0-9._-]/g,""))} placeholder="Ex.: 2674963"/></label></div>
