    ...
    {loading?<div className="communications-loading">Carregando configuração…</div>:<div className="integration-single-content">
      ...
      {section==="agenda"&&<><div className="integration-agenda-stack"><form className="communication-channel-card data-form" onSubmit={saveAgenda}>
          ...
          {isMaster&&canEdit&&<section className="communication-channel-card agenda-diagnostic-card">
            ...
            <button type="button" className="secondary full" disabled={agendaPreviewDisabled||busy==="lookup-agenda-family-chat"||!(agendaChatChannelId||agendaEduChannelId)||!agendaChatStudentId} onClick={()=>void lookupAgendaFamilyChat()}>{busy==="lookup-agenda-family-chat"?"Localizando chat…":"Localizar chat familiar sem criar"}</button>
            {agendaFamilyChat&&<div className="agenda-structure-results"><div className="agenda-structure-counts"><span><b>{agendaFamilyChat.chatId||"—"}</b> chat ID</span><span><b>{agendaFamilyChat.found?"Sim":"Não"}</b> encontrado</span><span><b>{agendaChatChannelId||agendaEduChannelId}</b> canal</span><span><b>{agendaChatStudentId}</b> aluno</span></div><small className="last-test">{agendaFamilyChat.attempted.replace(agendaFamilyChat.baseUrl,"GET ")}</small><details className={agendaFamilyChat.found?"agenda-diagnostic-probe ok":"agenda-diagnostic-probe error"} open><summary><span><b>Retorno do chat familiar</b><small>{agendaFamilyChat.found?"A API retornou um chat existente.":"A API não retornou chat existente para esse par canal/aluno."}</small></span><em>{agendaFamilyChat.found?"ok":"vazio"}</em></summary><pre>{JSON.stringify(agendaFamilyChat.raw,null,2)}</pre></details></div>}
          </section>}
          {isMaster&&canEdit&&<section className="communication-channel-card agenda-diagnostic-card">
