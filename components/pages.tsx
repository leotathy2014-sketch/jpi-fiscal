...
type AgendaStudentListItem={id:string;name:string;externalId:string|null;legacyId:string|null;mainClassroomId:string|null;period:string|null;status:string|null;linkedStatus:string|null;dateOfBirth:string|null};
type AgendaStudentListResult={message:string;baseUrl:string;environment:string;students:AgendaStudentListItem[];page:number;nextPage:number|null;totalPages:number|null;totalCount:number|null;attempted:string};
type AgendaResponsibleItem={id:string;name:string;externalId:string|null;legacyId:string|null;email:string|null;phone:string|null;documentNumber:string|null;kinship:string|null;financial:boolean;status:string|null;linkedStatus:string|null};
type AgendaClassroomItem={id:string;name:string;externalId:string|null;legacyId:string|null;status:string|null};
type AgendaChannelItem={id:string;name:string;type:string|null;status:string|null;visibility:string|null;kind:string|null;raw:unknown};
type AgendaChannelListResult={message:string;baseUrl:string;environment:string;channels:AgendaChannelItem[];page:number;nextPage:number|null;totalPages:number|null;totalCount:number|null;attempted:string};
type AgendaStudentDetailsResult={message:string;baseUrl:string;environment:string;student:AgendaStudentListItem|null;responsibles:AgendaResponsibleItem[];classrooms:AgendaClassroomItem[];primaryResponsible:AgendaResponsibleItem|null;included:unknown[];raw:unknown;attempted:string};
type AgendaFamilyChatResult={message:string;baseUrl:string;environment:string;chatId:string|null;found:boolean;attempted:string[];raw:unknown};
...
function CommunicationsSettings({accessToken,onChanged,canEdit,section}:{accessToken:string|null;onChanged:(config:CommunicationConfig)=>void;canEdit:boolean;section:"email"|"whatsapp"|"manual-whatsapp"|"agenda"}) {
  ...
  const [agendaEduEnvironment,setAgendaEduEnvironment]=useState("producao");
  ...
  const [agendaStudentList,setAgendaStudentList]=useState<AgendaStudentListResult|null>(null);
  const [agendaStudentPage,setAgendaStudentPage]=useState(1);
  const [agendaStudentDetails,setAgendaStudentDetails]=useState<AgendaStudentDetailsResult|null>(null);
  const [agendaChannelList,setAgendaChannelList]=useState<AgendaChannelListResult|null>(null);
  const [agendaChannelPage,setAgendaChannelPage]=useState(1);
  const [agendaChatChannelId,setAgendaChatChannelId]=useState("");
  const [agendaChatStudentId,setAgendaChatStudentId]=useState("");
  const [agendaFamilyChat,setAgendaFamilyChat]=useState<AgendaFamilyChatResult|null>(null);
  const [agendaChatStatus,setAgendaChatStatus]=useState("");
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      setAgendaEduEnvironment(current.agenda_edu_environment==="producao"?"producao":"homologacao");
  ...
  async function saveAgenda(event:FormEvent<HTMLFormElement>){event.preventDefault();setBusy("save-agenda");setError("");setMessage("");try{setMessage(await run("save-agenda",{schoolIdentifier:agendaEduSchoolIdentifier,channelId:agendaEduChannelId,environment:agendaEduEnvironment,clientId:agendaEduClientId,clientSecret:agendaEduClientSecret,schoolToken:agendaEduSchoolToken}));setAgendaEduClientId("");setAgendaEduClientSecret("");setAgendaEduSchoolToken("");await load();}catch(requestError){setError(requestError instanceof Error?requestError.message:"Não foi possível salvar a Agenda Edu.");}finally{setBusy("");}}
  async function setAgendaDefaultChannel(channel:AgendaChannelItem){setBusy(`agenda-default-channel-${channel.id}`);setError("");setMessage("");try{setAgendaEduChannelId(channel.id);setAgendaChatChannelId(channel.id);setMessage(await run("save-agenda",{schoolIdentifier:agendaEduSchoolIdentifier,channelId:channel.id,environment:agendaEduEnvironment,clientId:"",clientSecret:"",schoolToken:""}));await load();setMessage(`Canal padrão Agenda Edu definido: ${channel.name} (${channel.id}).`);}catch(requestError){setError(requestError instanceof Error?requestError.message:"Não foi possível definir o canal padrão da Agenda Edu.");}finally{setBusy("");}}
  ...
  async function listAgendaChannels(page=agendaChannelPage){setBusy("list-agenda-channels");setError("");setMessage("");try{if(!accessToken)throw new Error("Sessão expirada. Entre novamente.");const response=await authenticatedFetch("/api/integrations/communications",{method:"POST",headers:{Authorization:`Bearer ${accessToken}`,"Content-Type":"application/json"},body:JSON.stringify({action:"list-agenda-channels",page,perPage:50}),cache:"no-store"});const data=await response.json().catch(()=>({})) as AgendaChannelListResult&{error?:string};if(response.status===401)window.dispatchEvent(new Event("jpi-session-invalid"));if(!response.ok)throw new Error(data.error||"Não foi possível listar canais da Agenda Edu.");setAgendaChannelList(data);setAgendaChannelPage(data.page||page);setMessage(data.message||"Lista de canais Agenda Edu carregada. Nada foi gravado.");}catch(requestError){setError(requestError instanceof Error?requestError.message:"Não foi possível listar canais da Agenda Edu.");}finally{setBusy("");}}
  async function listAgendaStudents(page=agendaStudentPage){setBusy("list-agenda-students");setError("");setMessage("");try{if(!accessToken)throw new Error("Sessão expirada. Entre novamente.");const response=await authenticatedFetch("/api/integrations/communications",{method:"POST",headers:{Authorization:`Bearer ${accessToken}`,"Content-Type":"application/json"},body:JSON.stringify({action:"list-agenda-students",page,perPage:50}),cache:"no-store"});const data=await response.json().catch(()=>({})) as AgendaStudentListResult&{error?:string};if(response.status===401)window.dispatchEvent(new Event("jpi-session-invalid"));if(!response.ok)throw new Error(data.error||"Não foi possível listar alunos da Agenda Edu.");setAgendaStudentList(data);setAgendaStudentPage(data.page||page);setMessage(data.message||"Lista de alunos Agenda Edu carregada. Nada foi gravado.");}catch(requestError){setError(requestError instanceof Error?requestError.message:"Não foi possível listar alunos da Agenda Edu.");}finally{setBusy("");}}
  async function getAgendaStudentDetails(studentId:string){setBusy(`agenda-student-details-${studentId}`);setError("");setMessage("");setAgendaStudentDetails(null);try{if(!accessToken)throw new Error("Sessão expirada. Entre novamente.");const response=await authenticatedFetch("/api/integrations/communications",{method:"POST",headers:{Authorization:`Bearer ${accessToken}`,"Content-Type":"application/json"},body:JSON.stringify({action:"get-agenda-student-details",studentId}),cache:"no-store"});const data=await response.json().catch(()=>({})) as AgendaStudentDetailsResult&{error?:string};if(response.status===401)window.dispatchEvent(new Event("jpi-session-invalid"));if(!response.ok)throw new Error(data.error||"Não foi possível buscar detalhes do aluno.");setAgendaStudentDetails(data);setAgendaChatStudentId(data.student?.id||studentId);setMessage(data.message||"Detalhes do aluno carregados. Nada foi gravado.");}catch(requestError){setError(requestError instanceof Error?requestError.message:"Não foi possível buscar detalhes do aluno.");}finally{setBusy("");}}
  async function lookupAgendaFamilyChat(){setBusy("lookup-agenda-family-chat");setError("");setMessage("");setAgendaChatStatus("Consulta enviada para a Agenda Edu. Aguarde o retorno…");try{if(!accessToken)throw new Error("Sessão expirada. Entre novamente.");const channelId=agendaChatChannelId||agendaEduChannelId;const response=await authenticatedFetch("/api/integrations/communications",{method:"POST",headers:{Authorization:`Bearer ${accessToken}`,"Content-Type":"application/json"},body:JSON.stringify({action:"lookup-agenda-family-chat",channelId,studentId:agendaChatStudentId,useExternalId:false}),cache:"no-store"});const data=await response.json().catch(()=>({})) as AgendaFamilyChatResult&{error?:string};if(response.status===401)window.dispatchEvent(new Event("jpi-session-invalid"));if(!response.ok)throw new Error(data.error||"Não foi possível localizar o chat familiar.");setAgendaFamilyChat(data);setAgendaChatChannelId(channelId);setAgendaChatStatus(data.found?`Chat familiar encontrado: ${data.chatId}`:"A API respondeu, mas não retornou chat familiar existente para esse canal/aluno.");setMessage(data.message||"Consulta de chat familiar concluída. Nada foi gravado.");}catch(requestError){const text=requestError instanceof Error?requestError.message:"Não foi possível localizar o chat familiar.";setAgendaChatStatus(text);setError(text);}finally{setBusy("");}}
  ...
  return <div className="integration-detail-panel">
    ...
    {loading?<div className="communications-loading">Carregando configuração…</div>:<div className="integration-single-content">
      ...
      {section==="agenda"&&<><div className="integration-agenda-stack"><form className="communication-channel-card data-form" onSubmit={saveAgenda}>
          ...
          <div className="notice compact"><ShieldCheck/><span>Documentação oficial v2 confirmada. Troque o ambiente abaixo para usar as credenciais reais da plataforma oficial.</span></div>
          {isMaster&&<button type="button" className="agenda-api-info-button" onClick={()=>setAgendaApiInfoOpen(true)}><KeyRound/><span><strong>Informações da API e credenciais</strong><small>Consultar conexão, dados salvos no servidor e credenciais protegidas.</small></span><ArrowUpRight/></button>}
          ...
          <label>Ambiente da Agenda Edu<select value={agendaEduEnvironment} onChange={event=>setAgendaEduEnvironment(event.target.value)}><option value="producao">Plataforma oficial</option><option value="homologacao">Sandbox / homologação</option></select><small>Use Plataforma oficial com o novo Client ID e Client Secret informados pelo suporte.</small></label>
          <div className="smtp-server-summary"><span>Ambiente selecionado</span><strong>{agendaEduEnvironment==="producao"?"Plataforma oficial":"Sandbox / homologação"}</strong><small>{agendaEduEnvironment==="producao"?"api.agendaedu.com/v2":"sandbox-api.agendaedu.dev/v2"} · teste de conexão não envia mensagem.</small></div>
          ...
          {isMaster&&canEdit&&<section className="communication-channel-card agenda-diagnostic-card">
            <div className="communication-channel-head"><span className="integration-icon purple"><MessageCircle/></span><div><h3>Lista de canais Agenda Edu</h3><small>Defina a Secretaria Matriz como canal padrão do envio.</small></div><IntegrationStateBadge label={agendaChannelList?"Carregado":"Consulta"} tone={agendaChannelList?"connected":"pending"}/></div>
            <div className="notice compact"><ShieldCheck/><span>Consulta somente `GET /channels`. Use apenas o canal Secretaria Matriz, pois ele concentra alunos, turmas e responsáveis.</span></div>
            <div className="form-row"><label>Página<input inputMode="numeric" value={agendaChannelPage} onChange={event=>setAgendaChannelPage(Math.max(1,Number(event.target.value.replace(/\D/g,""))||1))}/></label><label>Quantidade por página<input value="50" readOnly/></label></div>
            <div className="form-actions"><button type="button" className="secondary" disabled={agendaPreviewDisabled||busy==="list-agenda-channels"||agendaChannelPage<=1} onClick={()=>void listAgendaChannels(Math.max(1,agendaChannelPage-1))}>Página anterior</button><button type="button" className="primary" disabled={agendaPreviewDisabled||busy==="list-agenda-channels"} onClick={()=>void listAgendaChannels(agendaChannelPage)}>{busy==="list-agenda-channels"?"Buscando canais…":"Buscar canais"}</button><button type="button" className="secondary" disabled={agendaPreviewDisabled||busy==="list-agenda-channels"||!agendaChannelList?.nextPage} onClick={()=>void listAgendaChannels(agendaChannelList?.nextPage||agendaChannelPage+1)}>Próxima página</button></div>
            {agendaChannelList&&<div className="agenda-structure-results"><div className="agenda-structure-counts"><span><b>{agendaChannelList.channels.length}</b> nesta página</span><span><b>{agendaChannelList.page}</b>{agendaChannelList.totalPages?` de ${agendaChannelList.totalPages}`:""} página(s)</span><span><b>{agendaChannelList.totalCount||"—"}</b> total API</span><span><b>{agendaChannelList.nextPage||"fim"}</b> próxima</span></div><small className="last-test">{agendaChannelList.attempted.replace(agendaChannelList.baseUrl,"GET ")}</small><div className="table-card"><table><thead><tr><th>ID canal</th><th>Nome</th><th>Tipo</th><th>Visibilidade</th><th>Status</th><th></th></tr></thead><tbody>{agendaChannelList.channels.map(channel=><tr key={channel.id}><td><strong>{channel.id}</strong></td><td><strong>{channel.name}</strong></td><td>{channel.kind||channel.type||"—"}</td><td>{channel.visibility||"—"}</td><td>{channel.status||"—"}</td><td><button type="button" className={agendaEduChannelId===channel.id?"primary mini":"secondary mini"} disabled={busy===`agenda-default-channel-${channel.id}`} onClick={()=>void setAgendaDefaultChannel(channel)}>{busy===`agenda-default-channel-${channel.id}`?"Salvando…":agendaEduChannelId===channel.id?"Canal padrão":"Definir padrão"}</button>{/secretaria.*matriz|matriz.*secretaria/i.test(channel.name)&&<span className="subcell">Secretaria Matriz</span>}</td></tr>)}</tbody></table></div><details className="agenda-diagnostic-probe ok"><summary><span><b>JSON dos canais</b><small>Primeiros itens retornados pela Agenda Edu</small></span><em>abrir</em></summary><pre>{JSON.stringify(agendaChannelList.channels.map(channel=>channel.raw),null,2)}</pre></details></div>}
          </section>}
          {isMaster&&canEdit&&<section className="communication-channel-card agenda-diagnostic-card">
            <div className="communication-channel-head"><span className="integration-icon blue"><UsersRound/></span><div><h3>Lista de alunos Agenda Edu</h3><small>Primeira etapa: carregar alunos da API oficial sem gravar vínculo.</small></div><IntegrationStateBadge label={agendaStudentList?"Carregado":"Consulta"} tone={agendaStudentList?"connected":"pending"}/></div>
            <div className="notice compact"><ShieldCheck/><span>Consulta somente `GET /student_profiles`. Use para conferir ID Agenda Edu, external_id e legacy_id antes dos detalhes.</span></div>
            <div className="form-row"><label>Página<input inputMode="numeric" value={agendaStudentPage} onChange={event=>setAgendaStudentPage(Math.max(1,Number(event.target.value.replace(/\D/g,""))||1))}/></label><label>Quantidade por página<input value="50" readOnly/></label></div>
            <div className="form-actions"><button type="button" className="secondary" disabled={agendaPreviewDisabled||busy==="list-agenda-students"||agendaStudentPage<=1} onClick={()=>void listAgendaStudents(Math.max(1,agendaStudentPage-1))}>Página anterior</button><button type="button" className="primary" disabled={agendaPreviewDisabled||busy==="list-agenda-students"} onClick={()=>void listAgendaStudents(agendaStudentPage)}>{busy==="list-agenda-students"?"Buscando alunos…":"Buscar lista de alunos"}</button><button type="button" className="secondary" disabled={agendaPreviewDisabled||busy==="list-agenda-students"||!agendaStudentList?.nextPage} onClick={()=>void listAgendaStudents(agendaStudentList?.nextPage||agendaStudentPage+1)}>Próxima página</button></div>
            {agendaStudentList&&<div className="agenda-structure-results"><div className="agenda-structure-counts"><span><b>{agendaStudentList.students.length}</b> nesta página</span><span><b>{agendaStudentList.page}</b>{agendaStudentList.totalPages?` de ${agendaStudentList.totalPages}`:""} página(s)</span><span><b>{agendaStudentList.totalCount||"—"}</b> total API</span><span><b>{agendaStudentList.nextPage||"fim"}</b> próxima</span></div><small className="last-test">{agendaStudentList.attempted.replace(agendaStudentList.baseUrl,"GET ")}</small><div className="table-card"><table><thead><tr><th>ID Agenda Edu</th><th>Aluno</th><th>External ID</th><th>Legacy ID</th><th>Turma principal</th><th>Status</th><th></th></tr></thead><tbody>{agendaStudentList.students.map(student=><tr key={student.id}><td><strong>{student.id}</strong></td><td><strong>{student.name}</strong><span className="subcell">{student.dateOfBirth||"Nascimento não informado"}</span></td><td>{student.externalId||"—"}</td><td>{student.legacyId||"—"}</td><td>{student.mainClassroomId||"—"}<span className="subcell">{student.period||""}</span></td><td>{student.status||"—"}<span className="subcell">{student.linkedStatus||""}</span></td><td><div className="form-actions compact-actions"><button type="button" className="secondary mini" onClick={()=>setAgendaChatStudentId(student.id)}>Usar aluno</button><button type="button" className="secondary mini" disabled={busy===`agenda-student-details-${student.id}`} onClick={()=>void getAgendaStudentDetails(student.id)}><Eye size={14}/>{busy===`agenda-student-details-${student.id}`?"Buscando…":"Detalhes"}</button></div></td></tr>)}</tbody></table></div>{agendaStudentDetails&&<details className="agenda-diagnostic-probe ok" open><summary><span><b>Prévia de envio · aluno {agendaStudentDetails.student?.id}</b><small>{agendaStudentDetails.student?.name||"Aluno retornado pela Agenda Edu"}</small></span><em>{agendaStudentDetails.responsibles.length} responsável(is) · {agendaStudentDetails.classrooms.length} turma(s)</em></summary><div className="agenda-structure-counts"><span><b>{agendaStudentDetails.student?.externalId||"—"}</b> external_id</span><span><b>{agendaStudentDetails.student?.legacyId||"—"}</b> legacy_id</span><span><b>{agendaStudentDetails.student?.mainClassroomId||"—"}</b> turma principal</span><span><b>{agendaStudentDetails.student?.linkedStatus||"—"}</b> vínculo</span></div><div className="notice compact"><MessageCircle/><span>Fluxo previsto: o JPI Fiscal localiza/cria o chat familiar do aluno <b>{agendaStudentDetails.student?.id}</b> e registra o responsável principal como referência fiscal.</span></div><div className="agenda-structure-issues"><strong>Responsável principal</strong>{agendaStudentDetails.primaryResponsible?<small><b>{agendaStudentDetails.primaryResponsible.name}</b> · {agendaStudentDetails.primaryResponsible.financial?"Financeiro":"Responsável"} · {agendaStudentDetails.primaryResponsible.kinship||"parentesco não informado"} · ID {agendaStudentDetails.primaryResponsible.id} · {agendaStudentDetails.primaryResponsible.linkedStatus||"sem status"}</small>:<small>Nenhum responsável retornado no detalhe do aluno.</small>}</div>{agendaStudentDetails.responsibles.length>0&&<div className="table-card"><table><thead><tr><th>Responsável</th><th>Tipo</th><th>Contato</th><th>Documento</th><th>Status</th></tr></thead><tbody>{agendaStudentDetails.responsibles.map(responsible=><tr key={responsible.id}><td><strong>{responsible.name}</strong><span className="subcell">ID {responsible.id} · {responsible.kinship||"parentesco não informado"}</span></td><td>{responsible.financial?"Financeiro":"Vinculado"}</td><td>{responsible.email||"Sem e-mail"}<span className="subcell">{responsible.phone||"Sem telefone"}</span></td><td>{responsible.documentNumber||"—"}<span className="subcell">{responsible.externalId||responsible.legacyId||""}</span></td><td>{responsible.status||"—"}<span className="subcell">{responsible.linkedStatus||""}</span></td></tr>)}</tbody></table></div>}<details className="agenda-diagnostic-probe ok"><summary><span><b>JSON completo sanitizado</b><small>Retorno bruto para conferência técnica</small></span><em>abrir</em></summary><pre>{JSON.stringify(agendaStudentDetails.raw,null,2)}</pre></details></details>}</div>}
          </section>}
          {isMaster&&canEdit&&<section className="communication-channel-card agenda-diagnostic-card">
            <div className="communication-channel-head"><span className="integration-icon green"><MessageCircle/></span><div><h3>Localizar chat familiar</h3><small>Usa o canal padrão Secretaria Matriz + aluno antes de criar mensagem.</small></div><IntegrationStateBadge label={agendaFamilyChat?agendaFamilyChat.found?"Encontrado":"Não encontrado":"Consulta"} tone={agendaFamilyChat?.found?"connected":"pending"}/></div>
            <div className="notice compact"><ShieldCheck/><span>Consulta somente `GET /channels/{'{'}canal{'}'}/chats`. Usa o canal padrão Secretaria Matriz. Não cria chat, não envia nota e não grava vínculo.</span></div>
            <div className="form-row"><label>ID do canal padrão<input value={agendaChatChannelId||agendaEduChannelId} onChange={event=>setAgendaChatChannelId(event.target.value.replace(/[^A-Za-z0-9._-]/g,""))} placeholder="Defina Secretaria Matriz acima"/><small>{agendaEduChannelId?`Canal padrão salvo: ${agendaEduChannelId}`:"Defina o canal Secretaria Matriz na lista de canais."}</small></label><label>ID do aluno Agenda Edu<input value={agendaChatStudentId} onChange={event=>setAgendaChatStudentId(event.target.value.replace(/[^A-Za-z0-9._-]/g,""))} placeholder="Ex.: 2674963"/></label></div>
            <button type="button" className="secondary full" disabled={agendaPreviewDisabled||busy==="lookup-agenda-family-chat"||!(agendaChatChannelId||agendaEduChannelId)||!agendaChatStudentId} onClick={()=>void lookupAgendaFamilyChat()}>{busy==="lookup-agenda-family-chat"?"Localizando chat…":"Localizar chat familiar sem criar"}</button>{agendaChatStatus&&<div className="notice compact"><Clock3/><span>{agendaChatStatus}</span></div>}
            {agendaFamilyChat&&<div className="agenda-structure-results"><div className="agenda-structure-counts"><span><b>{agendaFamilyChat.chatId||"—"}</b> chat ID</span><span><b>{agendaFamilyChat.found?"Sim":"Não"}</b> encontrado</span><span><b>{agendaChatChannelId||agendaEduChannelId}</b> canal</span><span><b>{agendaChatStudentId}</b> aluno</span></div><small className="last-test">{agendaFamilyChat.attempted.map(url=>url.replace(agendaFamilyChat.baseUrl,"GET ")).join(" | ")}</small><details className={agendaFamilyChat.found?"agenda-diagnostic-probe ok":"agenda-diagnostic-probe error"} open><summary><span><b>Retorno do chat familiar</b><small>{agendaFamilyChat.found?"A API retornou um chat existente.":"A API não retornou chat existente para esse par canal/aluno."}</small></span><em>{agendaFamilyChat.found?"ok":"vazio"}</em></summary><pre>{JSON.stringify(agendaFamilyChat.raw,null,2)}</pre></details></div>}
          </section>}
          {isMaster&&canEdit&&<section className="communication-channel-card agenda-diagnostic-card">
            ...
            {agendaDiagnostic&&<div className="agenda-diagnostic-results"><div className="agenda-diagnostic-summary"><strong>{agendaDiagnostic.baseUrl}</strong><span>{agendaDiagnostic.probes.filter(probe=>probe.ok).length} ok · {agendaDiagnostic.probes.filter(probe=>!probe.ok).length} com erro · {agendaDiagnostic.probes.filter(probe=>Number(probe.count)>0).length} com dados{agendaDiagnostic.studentName?` · nome: ${agendaDiagnostic.studentName}`:""}{agendaDiagnostic.sweducMatriculaId?` · matrícula: ${agendaDiagnostic.sweducMatriculaId}`:""}</span><small>{agendaDiagnostic.probes.some(probe=>probe.label.toLowerCase().includes("aluno")&&probe.status>=500)?"A Agenda Edu respondeu erro interno nos endpoints de aluno. Se os chats também vierem vazios, a API atual não está entregando lista de alunos para o canal.":"Abra cada teste abaixo para ver os campos reais retornados pela API."}</small></div>{agendaDiagnostic.probes.map((probe,index)=><details key={`${probe.label}-${index}`} className={probe.ok?"agenda-diagnostic-probe ok":"agenda-diagnostic-probe error"} open={index<2||Number(probe.count)>0}><summary><span><b>{probe.label}</b><small>{probe.method} {probe.endpoint}</small></span><em>{probe.status||"erro"}{probe.count!==null?` · ${probe.count} item(ns)`:""}</em></summary><pre>{JSON.stringify(probe.sample,null,2)}</pre></details>)}</div>}
          </section>}
        ...
        {isMaster&&agendaApiInfoOpen&&<div className="agenda-api-modal-backdrop" role="presentation" onMouseDown={event=>{if(event.currentTarget===event.target){setAgendaApiInfoOpen(false);hideAgendaMasterSecrets();}}}>
          <section className="agenda-api-modal" role="dialog" aria-modal="true" aria-labelledby="agenda-api-modal-title">
            <header className="agenda-api-modal-head">
              <div className="agenda-api-modal-title"><span><KeyRound/></span><div><small>ACESSO EXCLUSIVO DO MASTER</small><h2 id="agenda-api-modal-title">Informações da API e credenciais</h2><p>Dados salvos no servidor para consulta e manutenção futura da integração Agenda Edu.</p></div></div>
              <button type="button" className="agenda-api-modal-close" onClick={()=>{setAgendaApiInfoOpen(false);hideAgendaMasterSecrets();}} aria-label="Fechar"><X/></button>
            </header>

            <div className="agenda-api-modal-body">
              <section className="agenda-api-status-card">
                <div className="agenda-api-status-copy"><span className={config?.agenda_edu_ultimo_status==="conectado"?"connected":config?.agenda_edu_ultimo_status==="erro"?"error":"pending"}></span><div><small>STATUS DA INTEGRAÇÃO</small><strong>{config?.agenda_edu_ultimo_status==="conectado"?"Agenda Edu conectada":config?.agenda_edu_ultimo_status==="erro"?"Agenda Edu com erro":"Agenda Edu pendente"}</strong><p>{config?.agenda_edu_channel_id?"Canal de Mensagens identificado e salvo no servidor.":"Canal de Mensagens ainda não identificado."}</p></div></div>
                <span className="master-only-badge">MASTER</span>
              </section>

              <div className="agenda-api-modal-grid">
                <section className="agenda-api-info-section">
                  <div className="agenda-api-section-title"><Link2/><div><strong>Conexão e identificação</strong><small>Informações salvas no banco e utilizadas pela integração.</small></div></div>
                  <dl className="agenda-api-data-list">
                    <div><dt>Provedor</dt><dd>{config?.agenda_edu_provider||"agenda_edu"}</dd></div>
                    <div><dt>Ambiente</dt><dd>{config?.agenda_edu_environment==="producao"?"Plataforma oficial":config?.agenda_edu_environment==="homologacao"?"Sandbox / Homologação":config?.agenda_edu_environment||"Não informado"}</dd></div>
                    ...
                    <div><dt>Endpoint da API</dt><dd className="mono">{config?.agenda_edu_environment==="producao"?"api.agendaedu.com/v2":"sandbox-api.agendaedu.dev/v2"}</dd></div>
                ...
                <section className="agenda-api-info-section credentials">
                  ...
                  <div className="agenda-api-credential-list">
                    <div><label>Client ID</label><div><span className={agendaMasterSecretsVisible?"revealed":"masked"}>{agendaMasterSecretsVisible&&agendaMasterSecrets?.clientId?agendaMasterSecrets.clientId:"••••••••••••••••••••"}</span><button type="button" disabled={!agendaMasterSecretsVisible||!agendaMasterSecrets?.clientId} onClick={()=>void copyAgendaMasterSecret("clientId")} title="Copiar Client ID">{agendaMasterCopied==="clientId"?<Check/>:<Copy/>}</button></div></div>
                    <div><label>Client Secret</label><div><span className={agendaMasterSecretsVisible?"revealed":"masked"}>{agendaMasterSecretsVisible&&agendaMasterSecrets?.clientSecret?agendaMasterSecrets.clientSecret:"••••••••••••••••••••"}</span><button type="button" disabled={!agendaMasterSecretsVisible||!agendaMasterSecrets?.clientSecret} onClick={()=>void copyAgendaMasterSecret("clientSecret")} title="Copiar Client Secret">{agendaMasterCopied==="clientSecret"?<Check/>:<Copy/>}</button></div></div>
                    <div><label>X-School-Token</label><div><span className={agendaMasterSecretsVisible?"revealed":"masked"}>{agendaMasterSecretsVisible&&agendaMasterSecrets?.schoolToken?agendaMasterSecrets.schoolToken:"••••••••••••••••••••"}</span><button type="button" disabled={!agendaMasterSecretsVisible||!agendaMasterSecrets?.schoolToken} onClick={()=>void copyAgendaMasterSecret("schoolToken")} title="Copiar X-School-Token">{agendaMasterCopied==="schoolToken"?<Check/>:<Copy/>}</button></div></div>
                  </div>
                  <div className="agenda-api-secret-actions">
                    {!agendaMasterSecretsVisible?<button type="button" className="primary" onClick={()=>void revealAgendaMasterSecrets()} disabled={agendaMasterSecretsBusy||!config?.agenda_edu_credencial_configurada}><Eye/>{agendaMasterSecretsBusy?"Carregando…":"Revelar credenciais"}</button>:<button type="button" className="secondary" onClick={hideAgendaMasterSecrets}><EyeOff/>Ocultar credenciais</button>}
              ...
              <div className="agenda-api-security-note"><ShieldCheck/><div><strong>Proteção mantida</strong><span>Somente o perfil Master consegue abrir esta janela e solicitar os valores reais das credenciais ao servidor.</span></div></div>
            </div>
          </section>
        </div>}</>}    </div>}
  </div>;
