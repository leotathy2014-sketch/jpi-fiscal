...
export function IssuanceAssistant({onNavigate}:{onNavigate:(page:AppPage)=>void}){
  ...
  const steps=useMemo<AssistantStep[]>(()=>{
    ...
    const completion=[true,true,progress.validationDone,progress.dpsDone,progress.previewDone,progress.xmlDone,progress.sefinDone,progress.finished,Boolean(delivery)];
  ...
  const nextTitle=newEmissionOpen
    ...
    :selected?[
      "Aluno selecionado","Mensalidade criada",
      missing.length?"Corrigir cadastro antes de validar":"Validar dados da nota",
      "Preparar / editar DPS","Aprovar prévia da DPS","Gerar e validar XML","Enviar para homologação","Conferir nota concluída",
