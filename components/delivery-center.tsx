...
export function DeliveryCenter({role,accessToken}:{role:Role;accessToken:string|null}){
  ...
  function selectAll(){setSelected(new Set(filtered.filter(row=>Boolean(row.payment.alunos?.email)).map(row=>row.document.id)))}
  ...
  return <div className="delivery-page">
    ...
    {canManage&&filtered.length>0&&<section className="delivery-batch-bar"><div><strong>{selectedRows.length}</strong><span>selecionadas</span></div><button className="text-button" disabled={busy} onClick={selectAll}>Selecionar todas ({filtered.length})</button><button className="text-button" disabled={busy||!selected.size} onClick={()=>setSelected(new Set())}>Limpar</button><button className="primary" disabled={busy||!selectedRows.length} onClick={openConfirmation}><Send size={17}/>Enviar selecionadas</button></section>}
