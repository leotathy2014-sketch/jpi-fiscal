...
type SweducFinancial={numero_titulo?:string;valor?:string|number;situacao?:string;vencimento?:string;descricao?:string};
...
function financialValue(item:SweducFinancial){const value=item.valor;if(value===0)return "0";return value?String(value):"Valor não informado"}
function financialLabel(item:SweducFinancial,index:number){return item.numero_titulo||item.descricao||`Registro financeiro ${index+1}`}
...
export function SweducOperationalPicker({onStudentReady}:{onStudentReady:(student:{id:number;nome:string;turma?:string|null;segmento?:string;responsavel?:string;cpf_cnpj?:string|null;email?:string|null;whatsapp?:string|null;cep?:string|null;logradouro?:string|null;numero?:string|null;cidade?:string|null;uf?:string|null;sweduc_matricula_id?:number|null;sweduc_aluno_id?:number|null;sweduc_ano_letivo?:string|null})=>void}){
  ...
  const responsibleCard=selected?<div className="responsible-match-card sweduc-responsible-confirm" role="dialog" aria-label="Confirmar responsável financeiro SWeduc"><div><UserCheck/><span><strong>Confirmar responsável da nota</strong><small>{selected.nome} · matrícula {selected.numero_matricula||selected.matricula_id}</small></span></div>{busy===`details-${selected.matricula_id}`?<p>Carregando responsáveis vinculados a este aluno…</p>:selected.responsaveis.length>0?<div className="sweduc-responsible-options">{selected.responsaveis.map((responsible,index)=><label key={`${responsible.nome||"responsavel"}-${index}`} className="sweduc-responsible-option"><input type="radio" name={`sweduc-responsible-${selected.matricula_id}`} checked={responsibleIndex===index} onChange={()=>setResponsibleIndex(index)}/><span><strong>{responsible.nome||`Responsável ${index+1}`}{responsible.responsavel_pedagogico===true&&<em className="sweduc-suggested-responsible"><Check size={13}/>Sugerido pela SWeduc</em>}</strong><small>{responsibleDocument(responsible)} · {responsibleContact(responsible)}</small></span></label>)}</div>:<p>A SWeduc não retornou responsável para esta matrícula. Confira com o suporte antes de carregar para a nota.</p>}<div className="sweduc-financial-preview"><strong>Financeiro retornado pela SWeduc</strong>{selected.financeiro?.length?<div>{selected.financeiro.slice(0,5).map((item,index)=><span key={`${financialLabel(item,index)}-${index}`}><small>{financialLabel(item,index)}{item.situacao?` · ${item.situacao}`:""}{item.vencimento?` · venc. ${item.vencimento}`:""}</small><b>{financialValue(item)}</b></span>)}</div>:<small>Nenhum valor financeiro foi retornado para esta matrícula.</small>}</div><div><button type="button" className="secondary" onClick={()=>setSelected(null)}>Cancelar</button><button type="button" className="primary" disabled={Boolean(busy)||selected.responsaveis.length===0} onClick={()=>void loadForNote()}>{busy===`import-${selected.matricula_id}`?"Preparando…":<><Check size={15}/>Preparar para a nota</>}</button></div><small>Nada será gravado agora. O cadastro fiscal só será salvo quando a emissão for confirmada.</small></div>:null;
