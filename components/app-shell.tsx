...
export function AppShell({email,role,page,onPageChange,onSignOut}:{email:string;role:Role;page:AppPage;onPageChange:(p:AppPage)=>void;onSignOut:()=>void}) {
 ...
 useEffect(()=>{if(!supabase)return;supabase.from("certificado_a1_alerta").select("validade").eq("id",true).maybeSingle().then(({data})=>{setCertificateExpiry(data?.validade||null);setCertificateLoaded(true)})},[supabase]);
